...
#primary loading decreases 0.2
#no of loading differences = 16
library(MASS)
n <- c(50,200,600,1000)
final_result10 <- data.frame()
set.seed(45468)
...
for (i in 1:length(n)){
  ...
  result10 <- as.data.frame(t(replicate(50,getValue())))
  result10$size <- c(rep(n[i],50))
  result10$nrpcs <- 2
  result10$nrgroups <- 2
  final_result10 <- rbind(final_result10,result10)
}

#colnames(final_result10) <- c("Recovery","Tucker","Group_size")
save(final_result10, file = "final_result10v2.Rda")