...
for (i in 1:length(n)){
  ...
  result29 <- as.data.frame(t(replicate(50,getValue())))
  result29$size <- c(rep(n[i],50))
  result29$nrpcs <- 2
  result29$nrgroups <- 4
  final_result29 <- rbind(final_result29,result29)
}

#colnames(final_result29) <- c("Recovery","Tucker","Group_size")
save(final_result29, file = "final_result29v2.Rda")
