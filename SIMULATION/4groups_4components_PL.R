...
for (i in 1:length(n)){
  ...
  result37 <- as.data.frame(t(replicate(50,getValue())))
  result37$size <- c(rep(n[i],50))
  result37$nrpcs <- 4
  result37$nrgroups <- 4
  final_result37 <- rbind(final_result37,result37)
}

#colnames(final_result37) <- c("Recovery","Tucker","Group_size")
save(final_result37, file = "final_result37v2.Rda")
