...
for (i in 1:length(n)){
  ...
  result9 <- as.data.frame(t(replicate(50,getValue())))
  result9$size <- c(rep(n[i],50))
  result9$nrpcs <- 2
  result9$nrgroups <- 2
  final_result9 <- rbind(final_result9,result9)
}

#colnames(final_result9) <- c("Recovery","Tucker","Group_size")
save(final_result9, file = "final_result9v2.Rda")