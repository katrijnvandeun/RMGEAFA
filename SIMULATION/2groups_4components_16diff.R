...
for (i in 1:length(n)){
  ...
  result12 <- as.data.frame(t(replicate(50,getValue())))
  result12$size <- c(rep(n[i],50))
  result12$nrpcs <- 4
  result12$nrgroups <- 2
  final_result12 <- rbind(final_result12,result12)
}

#colnames(final_result12) <- c("Recovery","Tucker","Group_size")
save(final_result12, file = "final_result12v2.Rda")
