...
for (i in 1:length(n)){
  ...
  result5 <- as.data.frame(t(replicate(50,getValue())))
  result5$size <- c(rep(n[i],50))
  result5$nrpcs <- 2
  result5$nrgroups <- 2
  final_result5 <- rbind(final_result5,result5)
}

#colnames(final_result5) <- c("Recovery","Tucker","Group_size")
save(final_result5, file = "final_result5v2.Rda")
