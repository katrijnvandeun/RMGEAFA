...
for (i in 1:length(n)){
  getValue <- function(){
    ...
    #SP <- 1
    #ALPH <- 0.5
    #loadings <- INITIALLOADINGS(DATA, R, CARD=rep(7,4), SP, ALPH)
    #round(cbind(P1,loadings[[1]],P2,loadings[[2]]),2)
    ...
    #parameters<-parametersall[[1]]
    #parameters
    #round(cbind(P1,selmodel$loadings[[1]][,parametersall[[2]]],P2,selmodel$loadings[[2]][,parametersall[[2]]]),2)
    return(parameters)
  }
  
  result35 <- as.data.frame(t(replicate(50,getValue())))
  result35$size <- c(rep(n[i],50))
  result35$nrpcs <- 4
  result35$nrgroups <- 4
  final_result35 <- rbind(final_result35,result35)
}

#colnames(final_result35) <- c("Recovery","Tucker","Group_size")
save(final_result35, file = "final_result35v2.Rda")
