...
for (i in 1:length(n)){
  ...
  result14 <- as.data.frame(t(replicate(50,getValue())))
  result14$size <- c(rep(n[i],50))
  result14$nrpcs <- 4
  result14$nrgroups <- 2
  final_result14 <- rbind(final_result14,result14)
}

#colnames(final_result14) <- c("Recovery","Tucker","Group_size")
save(final_result14, file = "final_result14v2.Rda")
