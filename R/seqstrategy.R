...
#' @param ALPH
#' @param THR
#' @param TOL   rounding of PVE to TOL decimals
#'
#' @return
#' @export
seqstrategy <- function(DATA, R, SP = 0.9, ALPH = 0.1, THR = 0.03, TOL = 3) {
  ...
  #13.1.2026: misleading results! -> drop
  #cardvec <- 3:J
  #K <- length(cardvec)
  ...
  #!!!!!!!!!!!
  # what follows is selection by minimizing K among solutions with same PVE as max PVE
  #!!!!!!!!!!
  ...
  index <- indexnonzeroL
    ...
    if (is.na(indexnonzeroL) || min(avec[samepve, 2]) < selcardinality) {
      index <- placeholder[avec[, 2] == min(avec[samepve, 2])]
