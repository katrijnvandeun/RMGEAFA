seqstrategy <- function(DATA, R, SP=0.9, ALPH=0.1, THR=0.05, TOL=2){
  ...
  #TOL <- 2 #tolerance for pve expressed as rounding to TOL decimal places
  J <- dim(DATA[[1]])[2]
  eps <- 10^-6
  ...
  for (k in 1:K){
    ...
    avec[k,]<- c(lambdalow,cardvec[k],a$value,a$vaf,a$propzero,a$propunique,
  ...
  for (l in 1:L){
    ...
    avec[K+l,] <- c(lambdalow,cardvec[l],a$value,a$vaf,a$propzero,a$propunique,
                       a$smallestP,a$maxsdP)
  }
  round(avec,4)
  ...
  #selcardinality <- avec[avec[,"MinNonZeroL"]<THR,2][1]-1
