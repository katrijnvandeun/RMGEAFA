...
IS_JSPCA <- function(X, R,  SP, ALPH, lambda, card, MaxIter, eps){
  ...
  # PVEunreg = SSpca/SStotal with SSpca obtained from SVD with rank = R
  # PVEreg = 1-SSresidual/SStotal with SS residual from regularized model
#  ssrespca <- 0#needed for BIC but no longer used, keep in case asked
  ...
  for (g in 1:G){   
    ...
    #ssrespca <- ssrespca+sum((d[(R+1):length(d)])^2)#only BIC
  ...
  #############
  #bicvalue <- JSPCA$Residual/ssrespca+log(dfjspca)
  nrcoef <- J*R*G
