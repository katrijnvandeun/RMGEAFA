...
if (sys.nframe() == 0L) { # Check if the file is being sourced or run on its own; this influences the import paths
...
run_simulationG2R2 <- function() {
  results <- list()
  combs <- valid_args()[1:10,]
  combs_nr <- nrow(combs)
  ...
  for (i in 1:combs_nr) {
    ...
    print(comb)
    saveRDS(results[[i]], file=paste0(c("SimulationGeneratedFiles/resultG2R2_", i, ".rds"), collapse=""))
  }
  return(results)
}

run_simulationG2R4 <- function() {
  results <- list()
  combs <- valid_args()[11:20,]
  combs_nr <- nrow(combs)
  ...
  for (i in 1:combs_nr) {
    ...
    results[[i]] <- do.call(simulation, as.list(as.numeric(comb)))
    print(comb)
    saveRDS(results[[i]], file=paste0(c("SimulationGeneratedFiles/resultG2R4_", i, ".rds"), collapse=""))
  }
  return(results)
}

run_simulationG4R2 <- function() {
  results <- list()
  combs <- valid_args()[21:30,]
  combs_nr <- nrow(combs)
  ...
  for (i in 1:combs_nr) {
    ...
    results[[i]] <- do.call(simulation, as.list(as.numeric(comb)))
    print(comb)
    saveRDS(results[[i]], file=paste0(c("SimulationGeneratedFiles/resultG4R2_", i, ".rds"), collapse=""))
  }
  return(results)
}

run_simulationG4R4 <- function() {
  results <- list()
  combs <- valid_args()[31:40,]
  combs_nr <- nrow(combs)
  ...
  for (i in 1:combs_nr) {
    ...
    results[[i]] <- do.call(simulation, as.list(as.numeric(comb)))
    print(comb)
    saveRDS(results[[i]], file=paste0(c("SimulationGeneratedFiles/resultG4R4_", i, ".rds"), collapse=""))
  }
  return(results)
}
...
#'
#' @param resG2R2 A list of results, generated by 'run_simulationG2R2'
#' @param resG2R4 A list of results, generated by 'run_simulationG2R4'
#' @param resG4R2 A list of results, generated by 'run_simulationG4R2'
#' @param resG4R4 A list of results, generated by 'run_simulationG4R4'
#'
#' @return A dataframe with all results joined.
#' @export
join_simulation_results <- function(resG2R2, resG2R4, resG4R2, resG4R4) {
  condition_vec <- c(rep("PL_shift", 200),  rep("PL0.2", 200), rep("PL0.4", 200), rep("CL0.2", 200), rep("CL0.4", 200),
                     rep("PL_shift", 200),  rep("PL0.2", 200), rep("PL0.4", 200), rep("CL0.2", 200), rep("CL0.4", 200))
  diff_vec <- c(rep(4, 1000), rep(16, 1000))
    
  # Enrich results 
  dataG2R2 <- do.call(rbind, resG2R2)
  dataG2R2$condition <- condition_vec
  dataG2R2$diff <- diff_vec 
  
  dataG2R4 <- do.call(rbind, resG2R4)
  dataG2R4$condition <- condition_vec
  dataG2R4$diff <- diff_vec
  
  dataG4R2 <- do.call(rbind, resG4R2)
  dataG4R2$condition <- condition_vec
  dataG4R2$diff <- diff_vec 
  
  dataG4R4 <- do.call(rbind, resG4R4)
  dataG4R4$condition <- condition_vec
  dataG4R4$diff <- diff_vec 
